	...
	if(!class_exists('WooCommerce_Photo_Product_Tab')){
		class WooCommerce_Photo_Product_Tab{
			...
			public function photos_product_tabs_two($tabs){
				 ...
				 */
				$attachID = $wpdb->get_var("SELECT meta_value FROM wp_postmeta WHERE post_id = $post->ID AND meta_key = '_product_image_gallery'");
				if(empty($attachID)) return $tabs;
				$countPhotos = $wpdb->get_var("SELECT COUNT(*) FROM ".$wpdb->posts." WHERE `post_type`='attachment' AND `ID` IN ($attachID)");
			...
			public function photos_product_tabs_panel(){
				...
				if($countPhotos > 0 && get_post_meta($post->ID, 'woocommerce_disable_product_photos', true) != 'yes'){
					...
					if($attachments){
					  echo '<ul>';
					  	$attachments = explode(',', $attachID);
						foreach($attachments as $attachment){
							$photo_attr = array(
											'class'	=> "product-photo photo-attachment-".$attachment."",
							...
							if($woo_light == 'yes' && $photo_light == 'yes'){
								if(version_compare(WOOCOMMERCE_VERSION, "2.0", '<')){
									echo '<a href="'.wp_get_attachment_url($attachment).'" rel="prettyPhoto" class="zoom">';
								}
								else{
									echo '<a href="'.wp_get_attachment_url($attachment).'" rel="thumbnails" class="zoom">';
								}
							}
							echo wp_get_attachment_image($attachment, get_option('woocommerce_product_photo_tab_size'), false, $photo_attr);
