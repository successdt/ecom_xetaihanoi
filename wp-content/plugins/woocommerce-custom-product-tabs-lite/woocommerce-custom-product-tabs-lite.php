...
class WoocommerceCustomProductTabsLite {
	...
	public function product_write_panel_tab() {
		echo "<li class=\"product_tabs_lite_tab\"><a href=\"#woocommerce_product_tabs_lite\">" . __( 'Đặc tính kỹ thuật' ) . "</a></li>";
	}
	...
	public function product_write_panel() {
		...
		foreach ( $tab_data as $tab ) {
			...
			if (! isset( $tab['title'] ) || ! $tab['title'] ) $tab['title'] = 'Đặc tính kỹ thuật';
...
//		echo '<p class="form-field ' . $field['id'] . '_field"><label style="display:block;" for="' . $field['id'] . '">' . $field['label'] . '</label><textarea class="' . $field['class'] . '" name="' . $field['id'] . '" id="' . $field['id'] . '" placeholder="' . $field['placeholder'] . '" rows="2" cols="20"' . (isset( $field['style'] ) ? ' style="' . $field['style'] . '"' : '') . '>' . esc_textarea( $field['value'] ) . '</textarea> ';
		...
		wp_editor($field['value'], 'custom-tab', array('textarea_name' => $field['id'], 'textarea_rows' => 20) );
			//<textarea class="' . $field['class'] . '" name="' . $field['id'] . '" id="' . $field['id'] . '" placeholder="' . $field['placeholder'] . '" rows="2" cols="20"' . (isset( $field['style'] ) ? ' style="' . $field['style'] . '"' : '') . '>' . esc_textarea( $field['value'] ) . '</textarea> ';
